...
impl<'a, T> Syntree<T> {
    pub fn new(value: T, id: ID) -> Syntree<T> {
        /* */
        Syntree { id: (id), value: (value), children: (Vec::new()) }
        /* */
    }
    ...
    pub fn push_node(&mut self, parent_id: ID, new_node: Syntree<T>) -> Result<(), String> {
        ...
        match self.seek_node_mut(&parent_id){
            None => Err("ERROR: ID can`t be found!".to_string()),
            Some(Syntree { id, value, children }) => Ok(children.push(new_node)),
        }
        /* */
    }
    ...
    pub fn prepend_node(&mut self, parent_id: ID, new_node: Syntree<T>) -> Result<(), String> {
        ...
        match self.seek_node_mut(&parent_id){
            None => Err("ERROR: ID can`t be found!".to_string()),
            Some(Syntree { id, value, children }) => Ok(children.insert(0, new_node)),
        }
        /* */
    }
    ...
    pub fn insert_node(
        ...
        match self.seek_node_mut(&parent_id){
            None => Err("ERROR: ID can`t be found!".to_string()),
            Some(Syntree { id, value, children }) => Ok(children.insert(index, new_node)),
        }
        /* */
    }
